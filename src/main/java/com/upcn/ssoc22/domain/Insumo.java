...
public class Insumo implements Serializable {
    ...
    @ManyToMany(mappedBy = "excepcionInsumos")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "provisions", "excepcionInsumos", "contratoes" }, allowSetters = true)
    ...
    public void setPlans(Set<Plan> plans) {
        if (this.plans != null) {
            this.plans.forEach(i -> i.removeExcepcionInsumo(this));
        }
        if (plans != null) {
            plans.forEach(i -> i.addExcepcionInsumo(this));
        }
        this.plans = plans;
    }

    public Insumo plans(Set<Plan> plans) {
        this.setPlans(plans);
        return this;
    }

    public Insumo addPlan(Plan plan) {
        this.plans.add(plan);
        plan.getExcepcionInsumos().add(this);
        return this;
    }

    public Insumo removePlan(Plan plan) {
        this.plans.remove(plan);
        plan.getExcepcionInsumos().remove(this);
        return this;
    }
