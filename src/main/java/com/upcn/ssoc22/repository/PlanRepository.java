...
public interface PlanRepository extends JpaRepository<Plan, Long> {
    @Query(
        value = "select distinct jhiPlan from Plan jhiPlan left join fetch jhiPlan.prestacions",
        countQuery = "select count(distinct jhiPlan) from Plan jhiPlan"
    )
    Page<Plan> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct jhiPlan from Plan jhiPlan left join fetch jhiPlan.prestacions")
    List<Plan> findAllWithEagerRelationships();

    @Query("select jhiPlan from Plan jhiPlan left join fetch jhiPlan.prestacions where jhiPlan.id =:id")
