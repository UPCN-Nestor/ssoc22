...
public class ItemNomencladorResource {
    ...
    public List<ItemNomenclador> getAllItemNomencladors(@RequestParam(required = false) String filter) {
        if ("prestacion-is-null".equals(filter)) {
            log.debug("REST request to get all ItemNomencladors where prestacion is null");
            return StreamSupport
                .stream(itemNomencladorRepository.findAll().spliterator(), false)
                .filter(itemNomenclador -> itemNomenclador.getPrestacion() == null)
