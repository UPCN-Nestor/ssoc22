...
public class PlanResource {
    ...
    public ResponseEntity<Plan> partialUpdatePlan(@PathVariable(value = "id", required = false) final Long id, @RequestBody Plan plan)
        ...
        Optional<Plan> result = planRepository
            .findById(plan.getId())
            .map(existingPlan -> {
                if (plan.getTarifa() != null) {
                    existingPlan.setTarifa(plan.getTarifa());
                }
     ...
     */
    @GetMapping("/plans")
    public List<Plan> getAllPlans(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Plans");
        return planRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Plan> getPlan(@PathVariable Long id) {
        ...
        Optional<Plan> plan = planRepository.findOneWithEagerRelationships(id);
