...
class PlanResourceIT {
    ...
    void createPlan() throws Exception {
        ...
        assertThat(testPlan.getTarifa()).isEqualTo(DEFAULT_TARIFA);
    ...
    void getAllPlansWithEagerRelationshipsIsEnabled() throws Exception {
        when(planRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restPlanMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(planRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void putNewPlan() throws Exception {
        ...
        Plan testPlan = planList.get(planList.size() - 1);
        assertThat(testPlan.getTarifa()).isEqualTo(UPDATED_TARIFA);
